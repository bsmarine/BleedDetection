...
"""
   if cf.hold_out_test_set == False:
    ...
    #Concatenate detection raw boxes across folds
      det_frames = [pd.read_pickle(os.path.join(cf.exp_dir,f,'raw_pred_boxes_list.pickle')) for f in os.listdir(cf.exp_dir) if ('fold' in f) and (not '.pickle' in f)] 
      for i in det_frames:
        all_dets.extend(i)
    ...
    #Concatenate detection wbc boxes across folds
      det_frames = [pd.read_pickle(os.path.join(cf.exp_dir,f,'wbc_pred_boxes_list.pickle')) for f in os.listdir(cf.exp_dir) if 'fold' in f]
      for i in det_frames:
        all_dets.extend(i)
