...
class ProjectExciteLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        # print("Input tensor- batch size: {}, # channels: {}, D: {}, H: {}, W: {}".format(batch_size, num_channels, D, H, W))

        # Project:
        # Average along channels and different axes
        squeeze_tensor_w = F.adaptive_avg_pool3d(input_tensor, (1, 1, W))
        # print("Squeeze tensor (W) shape: ", squeeze_tensor_w.shape)

        squeeze_tensor_h = F.adaptive_avg_pool3d(input_tensor, (1, H, 1))
        # print("Squeeze tensor (H) shape: ", squeeze_tensor_h.shape)

        squeeze_tensor_d = F.adaptive_avg_pool3d(input_tensor, (D, 1, 1))
        # print("Squeeze tensor (D) shape: ", squeeze_tensor_d.shape)
        # print()
        ...
        # print("Final squeeze tensor shape (before excitation): ", final_squeeze_tensor.shape)
        
        # Excitation:
        final_squeeze_tensor = self.sigmoid(self.conv_cT(self.relu(self.conv_c(final_squeeze_tensor))))
        # print("Final squeeze tensor shape (after excitation): ", final_squeeze_tensor.shape)
        
        output_tensor = torch.mul(input_tensor, final_squeeze_tensor)
        # print("Output tensor (squeeze x input tensor): ", output_tensor.shape)
        # print()
