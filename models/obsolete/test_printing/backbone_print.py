...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.operate_stride1:
            ...
            # each with shape (b, c, y, x, (z))
        
        print("\nResNet PX_pre_out feature mape shapes from backbone.py")
        ...
        print("\nResNet feature map shapes from backbone.py")
        for count, ii in enumerate([c0_out, c1_out, c2_out, c3_out, c4_out, c5_out]):
            print ("encoder shapes for c{}:".format(count), ii.shape)
        print()

        for count, ii in enumerate([p5_out, p4_out, p3_out, p2_out]):
                print("decoder shapes for p{}:".format(5-count), ii.shape)
