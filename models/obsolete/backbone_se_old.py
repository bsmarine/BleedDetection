...
import torch
from squeeze_and_excitation import squeeze_and_excitation_3D_v as SE
...
class FPN(nn.Module):
    ...
    def __init__(self, cf, conv, operate_stride1=False):
        ...
        #        SE.ProjectExciteLayer(self.num_channels, self.SE_reduction_ratio)
        ...
        self.SE_block_c2 = SE_block(start_filts_exp, self.SE_reduction_ratio)
        self.SE_block_c3 = SE_block(start_filts_exp * 2, self.SE_reduction_ratio)
        self.SE_block_c4 = SE_block(start_filts_exp * 4, self.SE_reduction_ratio)
        self.SE_block_c5 = SE_block(start_filts_exp * 8, self.SE_reduction_ratio)

        # Post-interpolation/before PX_conv2 step (input: PX_pre_out)
        self.SE_block_preouts = SE_block(self.out_channels, self.SE_reduction_ratio)

        # After PX_conv2_step, acts on final outputs of layers of decoder block (input: PX_out)
        self.SE_block_outs = SE_block(self.out_channels, self.SE_reduction_ratio)
    ...
    def forward(self, x):
        ...
        #print('ResNet c1 layer')
        c1_out = self.C1(c0_out)
        
        #print('ResNet c2 layer')
        c2_out = self.C2(c1_out)
        
        #print('ResNet c3 layer')
        c3_out = self.C3(c2_out)
        
        #print('ResNet c4 layer')
        c4_out = self.C4(c3_out)
        
        # print('SE layer')
        #SE_layer1_out = self.SE_block(c4_out)
        
        #print('ResNet c5 layer')
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        out = self.conv1(x)
        #print("ResNet: after conv1, feature map shape: ", out.shape)
        
        out = self.conv2(out)
        #print("ResNet: after conv2, feature map shape: ", out.shape)
        
        out = self.conv3(out)
        #print("ResNet: after conv3, feature map shape: ", out.shape)
        
        if self.downsample:
            residual = self.downsample(x)
        out += residual
        out = self.relu(out)
        #print("ResNet: after relu, feature map shape: ", out.shape)
