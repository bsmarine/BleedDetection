...
import torch
from squeeze_and_excitation import squeeze_and_excitation_3D as SE
...
class FPN(nn.Module):
    ...
    # SE Blocks
        self.SE_reduction_ratio = cf.SE_reduction_ratio
        self.SE_block = SE.ProjectExciteLayer
        
        # Pre-interpolation/before PX_conv1 step (input: cX_out)
        self.cX_out_bool = False
        self.PX_pre_out_bool = False
        self.PX_out_bool = False
    
        se_placement_check = cf.se_placement.lower()
    
        if "cx_out" in se_placement_check:
            self.cX_out_bool = True
            self.SE_block_c2 = self.SE_block(start_filts_exp, self.SE_reduction_ratio)
            self.SE_block_c3 = self.SE_block(start_filts_exp * 2, self.SE_reduction_ratio)
            self.SE_block_c4 = self.SE_block(start_filts_exp * 4, self.SE_reduction_ratio)
            self.SE_block_c5 = self.SE_block(start_filts_exp * 8, self.SE_reduction_ratio)
                
        if self.sixth_pooling:
            self.SE_block_c6 = self.SE_block(start_filts_exp * 16, self.SE_reduction_ratio)

        # Post-interpolation/before PX_conv2 step (input: PX_pre_out)
        if "px_pre_out" in se_placement_check:
            self.PX_pre_out_bool = True

        # After PX_conv2_step, acts on final outputs of layers of decoder block (input: PX_out)
        if "px_out" in se_placement_check:
            self.PX_out_bool = True

        if self.PX_pre_out_bool or self.PX_out_bool:
            self.SE_block_preouts_outs = self.SE_block(self.out_channels, self.SE_reduction_ratio)
    #####
    ...
    def forward(self, x):
        ...
        if self.operate_stride1:
            ...
            c0_out = x
            print("c0_out (same as img) [any nans?]: {}\n".format(torch.any(torch.isnan(c0_out))))

        print("Pre SE modules:")
        
        print("Pre-conv c1 conv layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.C1[0].weight))))
        print("Pre-conv c1 conv layer weights min: {} ; max: {}\n".format(torch.min(self.C1[0].weight), torch.max(self.C1[0].weight)))

        c1_out = self.C1(c0_out)

        print("Post-conv c1_out [any nans?]: {}\n".format(torch.any(torch.isnan(c1_out))))
        #print(self.C1) 
        print("Post-conv c1 conv layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.C1[0].weight))))
        print("Post-conv c1 conv layer weights min: {} ; max: {}\n".format(torch.min(self.C1[0].weight), torch.max(self.C1[0].weight)))
        
        c2_out = self.C2(c1_out)
        print("Post-conv c2_out [any nans?]: {}".format(torch.any(torch.isnan(c2_out))))
        c3_out = self.C3(c2_out)
        print("Post-conv c3_out [any nans?]: {}".format(torch.any(torch.isnan(c3_out))))
        c4_out = self.C4(c3_out)
        print("Post-conv c4_out [any nans?]: {}".format(torch.any(torch.isnan(c4_out))))
        ...
        ## SE INSERT
        if self.cX_out_bool:
            print("\nInserted SE MODULES: cX_out")
            print("Pre-SE c2_out [any nans?]: {}".format(torch.any(torch.isnan(c2_out))))
            c2_out = self.SE_block_c2(c2_out)
            print("Post-SE c2_out [any nans?]: {}".format(torch.any(torch.isnan(c2_out))))

            print("Pre-SE c3_out [any nans?]: {}".format(torch.any(torch.isnan(c3_out))))
            c3_out = self.SE_block_c3(c3_out)
            print("Post-SE c3_out [any nans?]: {}".format(torch.any(torch.isnan(c3_out))))

            print("Pre-SE c4_out [any nans?]: {}".format(torch.any(torch.isnan(c4_out))))
            c4_out = self.SE_block_c4(c4_out)
            print("Post-SE c4_out [any nans?]: {}".format(torch.any(torch.isnan(c4_out))))

            print("Pre-SE c5_out [any nans?]: {}".format(torch.any(torch.isnan(c5_out))))
            c5_out = self.SE_block_c5(c5_out)
            print("Post-SE c5_out [any nans?]: {}\n".format(torch.any(torch.isnan(c5_out))))

            if self.sixth_pooling:
                c6_out = self.SE_block_c6(c6_out)
        ######
        ...
        p2_pre_out = self.P2_conv1(c2_out) + F.interpolate(p3_pre_out, scale_factor=2)
            ...
            #print("p4__pre_out [any nans?]: {}".format(torch.any(torch.isnan(p4_pre_out))))
            #print("p3__pre_out [any nans?]: {}".format(torch.any(torch.isnan(p3_pre_out))))
            #print("p2__pre_out [any nans?]: {}".format(torch.any(torch.isnan(p2_pre_out))))

### SE INSERT
        if self.PX_pre_out_bool:
            print("Inserted SE MODULES: pX_pre_out")
            p4_pre_out = self.SE_block_preouts_outs(p4_pre_out)
            p3_pre_out = self.SE_block_preouts_outs(p3_pre_out)
            p2_pre_out = self.SE_block_preouts_outs(p2_pre_out)
            
            if self.sixth_pooling:
                p5_pre_out = self.SE_block_preouts_outs(p5_pre_out)
        #####
        ...
        print("Pre-conv P2 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P2_conv2.weight))))
        print("Pre-conv P2 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P2_conv2.weight), torch.max(self.P2_conv2.weight)))

        print("Pre-conv P3 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P3_conv2.weight))))
        print("Pre-conv P3 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P3_conv2.weight), torch.max(self.P3_conv2.weight)))

        print("Pre-conv P4 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P4_conv2.weight))))
        print("Pre-conv P4 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P4_conv2.weight), torch.max(self.P4_conv2.weight)))

        print("Pre-conv P5 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P5_conv2.weight))))
        print("Pre-conv P5 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P5_conv2.weight), torch.max(self.P5_conv2.weight)))
       ...
       # print("Post-conv P2 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P2_conv2.weight))))
       # print("Post-conv P2 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P2_conv2.weight), torch.max(self.P2_conv2.weight)))

       # print("Post-conv P3 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P3_conv2.weight))))
       # print("Post-conv P3 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P3_conv2.weight), torch.max(self.P3_conv2.weight)))

        #print("Post-conv P4 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P4_conv2.weight))))
        #print("Post-conv P4 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P4_conv2.weight), torch.max(self.P4_conv2.weight)))

        #print("Post-conv P5 conv2 layer weights [any nans?]: {}".format(torch.any(torch.isnan(self.P5_conv2.weight))))
        #print("Post-conv P5 conv2 layer weights min: {} ; max: {}\n".format(torch.min(self.P5_conv2.weight), torch.max(self.P5_conv2.weight)))

        ### SE INSERT
        if self.PX_out_bool:
            print("Inserted SE MODULES: pX_out")
            p2_out = self.SE_block_preouts_outs(p2_out)
            p3_out = self.SE_block_preouts_outs(p3_out)
            p4_out = self.SE_block_preouts_outs(p4_out)
            p5_out = self.SE_block_preouts_outs(p5_out)
        #####
        ...
        if self.sixth_pooling:
            ...
            if self.PX_out_bool: # SE INSERT
                p6_out = self.SE_block_preouts_outs(p6_out)
        ...
        print("Final FPN outlist: p2_out [any nans?]: {}".format(torch.any(torch.isnan(out_list[0]))))
        print("Final FPN outlist: p3_out [any nans?]: {}".format(torch.any(torch.isnan(out_list[1]))))
        print("Final FPN outlist: p4_out [any nans?]: {}".format(torch.any(torch.isnan(out_list[2]))))
        print("Final FPN outlist: p5_out [any nans?]: {}\n".format(torch.any(torch.isnan(out_list[3]))))
        return out_list
